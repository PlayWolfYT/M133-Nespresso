...
async function query(query, callback = undefined) {
    return new Promise((resolve, reject) => {
        ...
        dbPool.getConnection((err, connection) => {
            if (err) {
                console.log("FATAL ERROR: Could not open Database Connection. Error: ", err);
                reject(err);
                return;
            }
...
async function setupTableStructures() {
    return new Promise((resolve, reject) => {
        queryFile(path.join(__dirname, "sql_files/database_structure.sql"), (err) => {
            if (err) {
                console.log(err);
                reject(err);
                return;
...
async function setupTableDefaults() {
    return new Promise((resolve, reject) => {
        queryFile(path.join(__dirname, "sql_files/database_defaults.sql"), (err) => {
            if (err) {
                console.log(err);
                reject(err);
                return;
